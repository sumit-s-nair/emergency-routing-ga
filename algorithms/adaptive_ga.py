...
This algorithm adjusts its mutation rate based on performance to balance
exploration and exploitation. It is based on the elitist GA structure.

Key Features:
- Adaptive Mutation Rate: Decreases on improvement, increases on stagnation.
...
class AdaptiveGA:
    """
    Adaptive Genetic Algorithm for Emergency Vehicle Dispatch and Routing Problem.
    
    Adjusts mutation rate based on convergence.
    ...
    def __init__(self, problem: Dict[str, Any], config: Dict[str, Any]):
        ...
        Args:
            problem: Dict containing problem data (see elitist_ga for details)
            config: Dict containing GA parameters
        """
        ...
        # --- Adaptive Parameters ---
        self.initial_mutation_rate = config.get('initial_mutation_rate', 0.1)
        self.min_mutation_rate = config.get('min_mutation_rate', 0.01)
        self.max_mutation_rate = config.get('max_mutation_rate', 0.5)
        # Factor to multiply/divide rate by
        self.adaptation_factor = config.get('adaptation_factor', 1.2)
        # Generations without improvement before increasing rate
        self.stagnation_limit = config.get('stagnation_limit', 20)
    ...
    def evolve_population(self, population: List[List[int]], 
                          ...
                          current_mutation_rate: float) -> List[List[int]]:
        """
        Evolve population for one generation using elitism and current mutation rate.
        ...
        Args:
            population: Current population
            fitness_values: Fitness values for each individual
            current_mutation_rate: The mutation rate to use for this generation
        ...
        while len(offspring) < offspring_needed:
            ...
            # Mutation (using the adaptive rate)
    ...
    def run_algorithm(self) -> Dict[str, Any]:
        ...
        for generation in range(self.generations):
            ...
            if not valid_indices:
                ...
                print(f"  Gen {generation:3d}: No valid solutions found, regenerating...")
                self.population = self.create_initial_population()
                self.stagnation_counter = 0 # Reset stagnation
                continue
            
            # Track best solution for this generation
            ...
            if min_fitness < self.best_fitness:
                ...
                # ADAPT: Decrease mutation rate (exploit)
            ...
            if self.stagnation_counter >= self.stagnation_limit:
                # ADAPT: Increase mutation rate (explore)
            ...
            mutation_rate_history.append(self.current_mutation_rate)
            ...
            if generation % 20 == 0 or generation == self.generations - 1:
                print(f"  Gen {generation:3d}: Best Fitness = {self.best_fitness:.4f}, "
                      f"Mut. Rate = {self.current_mutation_rate:.4f}, "
            ...
            self.population = self.evolve_population(
                self.population, 
                fitness_values,
                self.current_mutation_rate # Pass the current rate
            )
...
def run(problem=None, config=None):
    """
    Run Adaptive GA on the given problem.
    ...
    config.setdefault('initial_mutation_rate', 0.1)
    config.setdefault('min_mutation_rate', 0.01)
    config.setdefault('max_mutation_rate', 0.5)
    config.setdefault('adaptation_factor', 1.2)
    config.setdefault('stagnation_limit', 20)
    ...
    print(f"\n{'='*60}")
    print(f"Running Adaptive GA on dataset: {problem.get('dataset_name', 'unknown')}")
    print(f"{'='*60}")
    print(f"Config: pop_size={config['population_size']}, "
          f"generations={config['generations']}, "
          f"elite_size={config['elite_size']}")
    print(f"Adaptive Config: start_rate={config['initial_mutation_rate']}, "
          f"min_rate={config['min_mutation_rate']}, "
          f"max_rate={config['max_mutation_rate']}, "
          f"stagnation={config['stagnation_limit']}")

    
    # Run Adaptive GA
    try:
        ga = AdaptiveGA(problem, config)
        results = ga.run_algorithm()
        
        end = time.time()
        
        print(f"\n{'='*60}")
        print(f"Adaptive GA Results:")
        ...
        print(f"  Final Mut. Rate: {ga.current_mutation_rate:.4f}")
        ...
        return {
            ...
            "mutation_rate_history": results.get('mutation_rate_history', []),
        ...
        print(f"Error running Adaptive GA: {e}")
        import traceback
        traceback.print_exc()
        return {
            "algorithm": "adaptive_ga",
